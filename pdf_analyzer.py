...
def analyze_pdf_with_anthropic(api_key, pdf_file, model_choice="sonnet4"):
                ...
                messages=[
                    {
                        ...
                        "content": [
                            ...
                            {
                                "type": "text",
                                "text": """Analizza questo documento PDF e fornisci una descrizione dettagliata in italiano di ogni pagina.
...
def analyze_with_openai_text(api_key, pdf_text):
...
def create_zip_with_results(pdf_file, description_text, provider, model_name=None):
    ...
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        ...
        # Include model name in the description filename
        if model_name:
            description_filename = f"descrizione_{provider}_{model_name}_{pdf_file.name.replace('.pdf', '')}.txt"
        else:
            description_filename = f"descrizione_{provider}_{pdf_file.name.replace('.pdf', '')}.txt"
...
# Process button
if st.button("🚀 Analyze PDF", type="primary", disabled=not (uploaded_file and api_key)):
    if uploaded_file and api_key:
        ...
        if provider == "Anthropic (Native PDF)":
            ...
            with st.spinner("Extracting and analyzing text..."):
                ...
                if pdf_text:
                    description = analyze_with_openai_text(api_key, pdf_text)
        ...
        if description:
            ...
            # Show preview
            with st.expander("📋 Preview Description (first 2000 characters)"):
            ...
            # Include model name in the ZIP filename
            if model_name_for_file:
                zip_filename = f"pdf_analysis_{provider.split(' ')[0]}_{model_name_for_file}_{timestamp}.zip"
            else:
                zip_filename = f"pdf_analysis_{provider.split(' ')[0]}_{timestamp}.zip"
...
with st.expander("ℹ️ How to Use"):
    ...
    2. **Select Provider**: Anthropic for native PDF support, OpenAI for text-only
    3. **Choose Model**: 
    ...
    5. **Click Analyze**: Get detailed Italian description
    
    ### 💡 Tips
    - **Haiku 3.5** is perfect for simple documents
    - **Opus 4** for complex technical documents
    """)
